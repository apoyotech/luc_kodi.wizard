...
from .addonvar import setting, setting_set, addon_name, addon_icon, addon_id, addon_profile, isBase64, headers, dialog, local_string, addon_id, gui_save_default, kodi_ver_, get_page, thumbs, UPDATE_VERSION, CURRENT_BUILD, CURRENT_VERSION, BUILD_URL, BUILD_NAME, OLD_BUILD
...
def _maybe_show_local_changelog(addon_profile_path):
    try:
        import xbmcgui
        import xbmcaddon
        import os
        ...
        version_file = os.path.join(addon_profile_path, 'version.txt')
        last_version = None
        if os.path.exists(version_file):
            try:
                with open(version_file, 'r', encoding='utf-8') as _vf:
                    last_version = _vf.read().strip()
            except Exception:
                pass
        ...
        if current_version != last_version:
            ...
            if os.path.exists(changelog_path):
                try:
                    with open(changelog_path, 'r', encoding='utf-8') as _cf:
                        _msg = _cf.read()
                    # Intentar usar notify.notification_clog si existe
                    try:
                        from . import notify as _notify  # relativo al paquete
                        if hasattr(_notify, 'notification_clog'):
                            _notify.notification_clog(_msg)
                        else:
                            xbmcgui.Dialog().textviewer('Changelog ' + current_version, _msg)
                    except Exception:
                        xbmcgui.Dialog().textviewer('Changelog ' + current_version, _msg)
                except Exception:
                    xbmcgui.Dialog().textviewer('Changelog ' + current_version, 'No se pudo cargar el changelog.')
            # Guardar versión
            try:
                with open(version_file, 'w', encoding='utf-8') as _vf:
                    _vf.write(current_version)
            except Exception:
                pass
    except Exception:
...
class Startup:
           ...
           if UPDATE_VERSION is None:
               ...
               if UPDATE_VERSION > CURRENT_VERSION and setting('update_passed') != 'true' and CURRENT_BUILD == OLD_BUILD:
                   update_available = xbmcgui.Dialog().yesnocustom(
                       addon_name,
                       f'Update available! Your build has a name change!\nCurrent Name: [B]{CURRENT_BUILD}[/B]    New Name: [B]{BUILD_NAME}[/B]\n{local_string(30049)} [B]{CURRENT_VERSION}[/B]    {local_string(30050)} [B]{UPDATE_VERSION}[/B]\n{local_string(30051)}',
                   ...
                   update_available = xbmcgui.Dialog().yesnocustom(
                       addon_name,
                       f'{local_string(30047)} [B]{CURRENT_BUILD}[/B] {local_string(30048)}\n{local_string(30049)} [B]{CURRENT_VERSION}[/B]\n{local_string(30050)} [B]{UPDATE_VERSION}[/B]\n{local_string(30051)}',
                       yeslabel='Update Now', nolabel='Not Now', customlabel='View Changelog', defaultbutton=xbmcgui.DLG_YESNO_CUSTOM_BTN
                   )
        ...
        if not xbmcvfs.exists(file_path) and current_list:  #Check if build_list.json exists and current list is not empty
            ...
            with open(file_path, 'r') as archivedlist:
                ...
                if current_list == archived_list:  #Check if lists are identical
                    ...
                    notify = xbmcgui.Dialog().yesnocustom(addon_name, f'New build(s) available! Would you like to check them out?\n\n[B]{new_builds}[/B]', yeslabel='Yes', nolabel='No', customlabel='Remind Me Later', defaultbutton=xbmcgui.DLG_YESNO_YES_BTN)
                    if notify == 1:     #Yes
                        with open(file_path, 'w') as buildlist:
                            json.dump({'builds': current_list}, buildlist, indent = 4)   #Update archived list
    ...
    def run_startup(self):
        ...
        if setting('firstrun') == 'true':
            ...
            if thumbs > 500:
                xbmcgui.Dialog().notification(addon_name, f'Thumbnails folder is [COLOR red]{thumbs}MB[/COLOR]', addon_icon, 3000)
            xbmc.sleep(1000)
            utils.pbf()
            _maybe_show_local_changelog(addon_profile)
