...
# --- Network helper: timeouts & retries ---
import socket
def fetch_text(url, headers, timeout=12, retries=2):
    """Devuelve el cuerpo decodificado en 'utf-8' con reintentos.
    Lanza la última excepción si falla todo.
    """
    last = None
...
def file_check(bfile):
    ...
    return bfile
def get_page(url):
    # normaliza enlaces (base64/dropbox, etc.) y aplica timeout/reintentos
    return fetch_text(file_check(url), headers, timeout=12, retries=2)
def splash_chk():
